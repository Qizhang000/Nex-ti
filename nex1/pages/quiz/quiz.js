...
function getClosestRoleKey(scores) {
  let bestKey = null;
  let bestDistance = Number.POSITIVE_INFINITY;

  Object.keys(roleProfiles).forEach((key) => {
    const prototype = roleProfiles[key].prototype;
    const distance = dimensions.reduce((sum, dim) => sum + Math.abs(scores[dim.key] - prototype[dim.key] * 5), 0);
...
Page({
  ...
  answerQuestion(e) {
    ...
    const { currentIndex, questions: questionList, scores, answerHistory } = this.data;
    const pickedScore = questionList[currentIndex].options[optionIndex].score;
    const nextScores = { ...scores };

    Object.keys(pickedScore).forEach((dim) => {
      nextScores[dim] += pickedScore[dim];
    });
    ...
    this.showResult(nextScores, nextHistory);
  },

  goToPreviousQuestion() {
    const { currentIndex, answerHistory, questions: questionList, scores } = this.data;
    ...
    const lastAnswerIndex = answerHistory[answerHistory.length - 1];
    const lastScore = questionList[previousIndex].options[lastAnswerIndex].score;
    const nextScores = { ...scores };

    Object.keys(lastScore).forEach((dim) => {
      nextScores[dim] -= lastScore[dim];
    });
  ...
  showResult(finalScores, answerHistory) {
    const roleKey = getClosestRoleKey(finalScores);
    const role = roleProfiles[roleKey];
    const dimensionBreakdown = dimensions.map((dim) => {
      const value = finalScores[dim.key];
      const polarity = value >= 0 ? dim.pos : dim.neg;
      const sign = value >= 0 ? "+" : "";
      return `${dim.label}: ${polarity}（${sign}${value}）`;
    });
    ...
    this.setData({
      ...
      showResult: true,
      resultRole: role,
      resultDescription: `${role.summary} ${role.detail}`,
      dimensionBreakdown
    }, () => {
      this.drawRadarChart(finalScores, role.name);
    });
  },

  drawRadarChart(finalScores, roleName) {
    ...
    ctx.beginPath();
    dimensions.forEach((dim, idx) => {
      const value = getNormalizedValue(finalScores[dim.key]);
    ...
    ctx.closePath();
    ctx.setFillStyle("rgba(236, 72, 153, 0.28)");
    ctx.setStrokeStyle("#db2777");
    ctx.setLineWidth(2);
    ctx.fill();
    ctx.stroke();
    ...
    ctx.fillText(`匹配成员: ${roleName}`, 16, 24);
    ctx.draw();
  },
